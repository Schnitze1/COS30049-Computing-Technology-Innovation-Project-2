...
def save_results_csv(results: Dict[str, Dict[str, float]], out_dir: str = None,
...
def plot_confusion_matrices(results: Dict[str, Dict[str, float]],
    ...
    model_confusion_matrices = [(model_name, res['confusion_matrix'])
    ...
    for idx, (model_name, confusion_matrix) in enumerate(
            model_confusion_matrices, start=1):
        ax = plt.subplot(rows, cols, idx)
        # Determine if binary or multiclass
        if hasattr(confusion_matrix, 'shape') and confusion_matrix.shape == (2, 2):
            labels = (['False', 'True'] if class_labels is None
                      else class_labels[:2])
...
def plot_multiclass_metrics(results: Dict[str, Dict[str, float]],
                            out_dir: str = 'evaluation_reports/multiclass') -> str:
    """Create comprehensive multiclass metrics visualization."""
    results_df = results_to_dataframe(results)

    # Define multiclass metrics to plot
    multiclass_metrics = ['accuracy', 'precision_weighted', 'recall_weighted',
...
def plot_per_class_metrics(results: Dict[str, Dict[str, float]],
...
def export_reports(results: Dict[str, Dict[str, float]], traffic_types: List[str],
    ...
    per_class_paths = plot_per_class_metrics(results, traffic_types,
                                             multiclass_out_dir)
...
def export_clustering_reports(models: Dict[str, object], x_data: np.ndarray,
                              y_true: np.ndarray, traffic_types: List[str],
    ...
    for clustering_model in ['kmeans', 'dbscan']:
        if clustering_model in models:
            ...
            if clustering_model == 'kmeans':
                y_clusters = model.predict(x_data)
            else:
                y_clusters = model.fit_predict(x_data)

            # Generate PCA scatter plot
            pca_path = plot_clustering_pca_scatter(
                x_data, y_clusters, clustering_model, out_dir)
...
def plot_clustering_pca_scatter(x_data: np.ndarray, y_clusters: np.ndarray,
                                algorithm_name: str,
                                out_dir: str = 'evaluation_reports/clustering'
                                ) -> str:
    ...
    scatter = plt.scatter(features_2d[:, 0], features_2d[:, 1], c=y_clusters,
    ...
    if algorithm_name.lower() == 'dbscan' and -1 in unique_clusters:
        ...
        cbar = plt.colorbar(scatter, boundaries=np.arange(
            len(unique_clusters) + 1) - 0.5)
        cbar.set_label('Cluster ID (Noise=-1)')
    else:
        # Standard clustering (K-means, etc.)
        cbar = plt.colorbar(scatter, boundaries=np.arange(
            int(np.max(y_clusters)) + 2) - 0.5)
...
def plot_cluster_label_heatmap(y_clusters: np.ndarray, y_true: np.ndarray,
                               traffic_types: List[str], algorithm_name: str,
                               out_dir: str = 'evaluation_reports/clustering'
                               ) -> str:
    ...
    if algorithm_name.lower() == 'dbscan' and -1 in unique_clusters:
        ...
        cluster_labels = [f'C{cluster_id}' if cluster_id != -1 else 'Noise'
    ...
    for row_index, cluster_id in enumerate(unique_clusters):
        ...
        if member_indices.size == 0:
            continue
        unique_true_labels, label_counts = np.unique(
            y_true[member_indices], return_counts=True)
        for label, count in zip(unique_true_labels, label_counts):
            cluster_vs_label_counts[row_index, int(label)] = int(count)

    plt.figure(figsize=(max(8, len(traffic_types)*0.9),
                max(5, n_clusters*0.4)))
    sns.heatmap(cluster_vs_label_counts, annot=False, fmt='d', cmap='Purples',
                cbar=True, yticklabels=cluster_labels,
