...
def model_architecture(model_name: str, top_k: int = 5) -> dict:
    ...
    if not hasattr(model, "coefs_"):
        raise HTTPException(status_code=400, detail={"message": f"Model '{model_name}' has no accessible architecture"})
    ...
    for i, weights in enumerate(model.coefs_):
        ...
        for j in range(weights.shape[1]):
            ...
            for source_i in sorted_i:
                edges.append({"source_i": int(source_i), "target_i": int(j), "weight": float(weights[source_i, j])})
